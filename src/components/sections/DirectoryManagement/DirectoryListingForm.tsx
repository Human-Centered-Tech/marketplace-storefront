...
export const DirectoryListingForm = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Service area is edited on its own page now (/user/directory/service-area,
    // which writes through the dedicated endpoint) — but this form MUST keep
    // sending the value it loaded. The backend PUT replaces `address`
    // wholesale (only owner_interview/devotional are merged), so dropping the
    // key here would silently clear the merchant's serviced states every time
    // they saved anything else on the listing.
    //
    // The home-state fallback stays for listings still created through this
    // form: an address alone keeps them filterable instead of invisible. In
    // edit mode an empty set is respected as a deliberate clear.
